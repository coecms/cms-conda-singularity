...
function inner() {
    ...
    cp "${SCRIPT_DIR}"/condaenv.sh "${CONDA_MODULE_PATH}"/"${MODULE_NAME}"
    ...
    copy_and_replace "${SCRIPT_DIR}"/../modules/are       "${CONDA_MODULE_PATH}"/"${MODULE_NAME}"/are        CONDA_BASE APPS_SUBDIR CONDA_INSTALL_BASENAME
