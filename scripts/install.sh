...
function inner() {
    ...
    if [[ "${1}" == "--install" ]]; then
        ...
        rm -rf "${CONDA_BASE}"/scripts/"${FULLENV}".d/{bin,overrides}
    fi
...
rsync --recursive --links --perms --times --specials --partial --quiet --one-file-system --hard-links --acls --exclude=*.sqsh -- "${CONDA_BASE}"/{apps,modules,scripts} "${CONDA_OUTER_BASE}"/
echo "Done"
...
if [[ -e  "${CONDA_INSTALLATION_PATH}/envs/${FULLENV}.sqsh" ]]; then
    ...
    popd
    rm "${CONDA_OUTER_BASE}"/apps/miniconda3/envs/"${FULLENV}"
...
read oldhash fn < <( md5sum "${CONDA_OUTER_BASE}"/apps/miniconda3/etc/"${CONTAINER_PATH##*/}" )
if [[ "${oldhash}" != "${newhash}" ]]; then
    echo "Container update detected. Copying in new container"
    cp "${CONTAINER_PATH}" "${CONDA_OUTER_BASE}"/apps/miniconda3/etc/"${CONTAINER_PATH##*/}"
fi
...
if [[ "${DO_UPDATE}" == "--install" ]]; then
    ln -s .common_v3 "${CONDA_OUTER_BASE}"/modules/conda/"${FULLENV}"
fi
...
rm "${CONDA_OUTER_BASE}"/apps/miniconda3/envs/"${FULLENV}"
ln -s /opt/conda/"${FULLENV}" "${CONDA_OUTER_BASE}"/apps/miniconda3/envs/
...
rsync --archive --verbose --partial --progress --one-file-system --itemize-changes --hard-links --acls -- "${CONDA_OUTER_BASE}"/{apps,modules,scripts} "${CONDA_BASE}"

echo "Make sure anything deleted from the base conda environment is also deleted in the prod copy"
rsync --archive --verbose --partial --progress --one-file-system --itemize-changes --hard-links --acls --delete --exclude=*.sqsh -- "${CONDA_OUTER_BASE}"/apps/miniconda3/ "${CONDA_INSTALLATION_PATH}"

echo "Make sure anything deleted from the scripts directory is also deleted from the prod copy"
rsync --archive --verbose --partial --progress --one-file-system --itemize-changes --hard-links --acls --delete -- "${CONDA_OUTER_BASE}"/scripts/ "${CONDA_BASE}"/scripts
